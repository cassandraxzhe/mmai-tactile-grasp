"""Distributed training utilities for torchrun and SLURM."""
...
def init_distributed_device(args):
    """Sets args.distributed/world_size/rank/local_rank/device. Returns torch.device."""
...
def init_distributed_device_so(
    ...
    """Standalone distributed init. Supports torchrun and SLURM."""
    distributed = False
    world_size = 1
    global_rank = 0
    local_rank = 0
    ...
    if not is_known:
        warnings.warn(f"Device {device} was not known, trying anyways.")
    elif not is_avail:
    ...
    if is_using_distributed():
        if dist_backend is None:
            dist_backends = {"cuda": "nccl", "hpu": "hccl", "npu": "hccl", "xpu": "ccl"}
    ...
    return dict(
        device=device, global_rank=global_rank,
